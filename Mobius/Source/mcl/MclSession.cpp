 ...
 * Incremental Parsing
 *
 * Each line needs to do something:
 *
 *     session foo
 *
 * This reads or creates the Session identified by name
 */
...
#include "MclModel.h"
#include "MclResult.h"
#include "MclSession.h"

MclSession::MclSession(Provider* p)
{
    provider = p;
}

MclSession::~MclSession()
{
}

void MclSession::parse(juce::String src, MclResult& userResult)
{
...
void MclSession::addError(juce::String err)
{
    result->errors.add(juce::String("Line ") + juce::String(lineNumber) + ": " + line);
    result->errors.add(err);
}

bool MclSession::hasErrors()
{
    return result->errors.size() > 0;
}

MclSection* MclSession::getSection()
{
...
void MclSession::parseLine()
{
    ...
    bool traceit = false;
    if (traceit) {
        Trace(2, "MclSession line %s", line.toUTF8());
        for (auto t : tokens)
          Trace(2, "  %s", t.toUTF8());
    }
...
void MclSession::parseSession(juce::StringArray& tokens)
{
    if (tokens.size() > 2) {
...
void MclSession::parseOverlay(juce::StringArray& tokens)
{
    if (tokens.size() < 2) {
...
void MclSession::parseSessionLine(juce::StringArray& tokens, bool isOverlay)
{
    if (tokens.size() > 0) {
...
void MclSession::parseScope(juce::StringArray& tokens)
{
    if (tokens.size() != 2) {
...
int MclSession::parseScopeId(juce::String token)
{
    int trackNumber = 0;
...
void MclSession::parseAssignment(juce::StringArray& tokens)
{
 ...
 */
int MclSession::parseParameterOrdinal(Symbol* s, ParameterProperties* props,
 ...
 */
void MclSession::validateStructureReference(Symbol* s, ParameterProperties* props,
                                           juce::String svalue)
{
 ...
 */
bool MclSession::isKeyword(juce::String token, const char* keyword)
{
 ...
 */
void MclSession::parseBinding(juce::StringArray& tokens)
{
    if (tokens.size() < 2) {
...
void MclSession::parseBindingLine(juce::StringArray& tokens)
{
    if (tokens.size() > 0) {
        juce::String keyword = tokens[0];
        if (keyword == "default")
...
void MclSession::parseBindingDefault(juce::StringArray& tokens)
{
...
void MclSession::parseBindingColumns(juce::StringArray& tokens)
{
    int column = 1;
...
int MclSession::parseChannel(juce::String s)
{
...
Trigger* MclSession::parseTrigger(juce::String s)
{
...
int MclSession::parseMidiValue(juce::String s)
{
...
void MclSession::parseBindingObject(juce::StringArray& tokens)
{
...
bool MclSession::validateSymbol(juce::String name)
{
    bool valid = false;
...
bool MclSession::validateBindingScope(juce::String name)
{
    (void)name;
