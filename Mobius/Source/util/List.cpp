 ...
 * elements.
 *
 * ...Well it's 2024 now and it's time to enbrace change
 * Can completely get rid of this and use std::vector
 * lol, you couldn't bring yourself to use std::vector except
...
// rare use of std:: for vector sorting
#include <vector>
#include <algorithm>
...
void List::initList(int initialSize)
{
	mElements = NULL;
	mArraySize = 0;
	mSize = 0;
...
void StringList::sort()
{
    if (mSize > 0) {
        ...
        std::sort(v->begin(), v->end(), StringSorter);
