...
XmlMiniParser::XmlMiniParser(void)
{
	mPreserveCharent	= 0;
	mInlineEntref		= 0;
	mFilterComments		= 0;
...
void XmlMiniParser::setFile(const char *name)
{
	if (name == NULL) {
		delete mInputFile;
		mInputFile = NULL;
...
void XmlMiniParser::setBuffer(const char *buffer,
										int length)
{
	mInputBuffer = buffer;
...
void XmlMiniParser::throwException(int code, const char *more)
{
	Vbuf *buf;
	char *msg;
	int addpsn;
...
void XmlEventAdapter::openDoctype(XmlMiniParser *p, 
											char *name, 
											char *pubid,
											char *sysid)
{
...
void XmlEventAdapter::closeDoctype(XmlMiniParser *p)
{
	p=p;
}

void XmlEventAdapter::openStartTag(XmlMiniParser *p, 
											 char *name)
{
	p=p;
	delete name;
...
void XmlEventAdapter::attribute(XmlMiniParser *p, 
										  char *name, 
										  char *value)
{
	p=p;
...
void XmlEventAdapter::closeStartTag(XmlMiniParser *p, int empty)
{
	p=p;
	empty=empty;
}

void XmlEventAdapter::endTag(XmlMiniParser *p, char *name)
{
	p=p;
	delete name;
}

void XmlEventAdapter::comment(XmlMiniParser *p, char *text)
{
	p=p;
	delete text;
}

void XmlEventAdapter::pi(XmlMiniParser *p, char *text)
{
	p=p;
	delete text;
}

void XmlEventAdapter::pcdata(XmlMiniParser *p, char *text)
{
	p=p;
	delete text;
}

void XmlEventAdapter::entref(XmlMiniParser *p, char *name)
{
	p=p;
	delete name;
}

void XmlEventAdapter::cdata(XmlMiniParser *p, char *text)
{
	p=p;
	delete text;
}

void XmlEventAdapter::error(XmlMiniParser *p, int code, 
									  const char *msg)
{
...
void XmlMiniParser::openInput(void)
{
    if (mInputIsOpen)
        return;
...
void XmlMiniParser::closeInput(void)
{
	if (mInputFp != NULL) {
		fclose(mInputFp);
		mInputFp = NULL;
...
char XmlMiniParser::nextchar(void)
{
	if (mEof)
...
char XmlMiniParser::advance(void)
{
	if (!mEof) {
		if (mInputBuffer != NULL) {
...
char XmlMiniParser::advanceToChar(void)
{
	char ch;
...
void XmlMiniParser::addLookahead(void)
{
...
void XmlMiniParser::shiftLookahead(int max)
{
...
int XmlMiniParser::compareLookahead(const char *pattern)
{
...
void XmlMiniParser::clearLookahead(void)
{
	mLookaheadIndex = 0;
}
...
void XmlMiniParser::saveLookahead(void)
{
	int i;
...
void XmlMiniParser::clearToken(void)
{
	mTokenIndex = 0;
	// always leave mTokbuf null terminated
	mTokbuf[0] = '\0';
...
void XmlMiniParser::addData(int ch)
{
	mDatabuf->addChar((const char)ch);
}

void XmlMiniParser::addData(const char *str)
{
	mDatabuf->add(str);
}
...
char *XmlMiniParser::getData(void)
{
	char *data = NULL;
...
void XmlMiniParser::finishData(void)
{
	char *data;
...
void XmlMiniParser::clearData(void)
{
	mDatabuf->clear();
}
...
void XmlMiniParser::syntaxError(void)
{
...
void XmlMiniParser::consumeKeyword(const char *expected)
{
	const char *ptr;
	int ch;
...
void XmlMiniParser::parseTagOpen(void)
{
	int ch;
...
void XmlMiniParser::parseCloseBracket(void)
{
	int ch;
...
void XmlMiniParser::parseEntityName(void)
{
	int ch;
...
char *XmlMiniParser::copyString(const char *src)
{
	char *copy = nullptr;
	if (src != NULL) {
...
void XmlMiniParser::parseEntref(void)
{
	int ch;
...
void XmlMiniParser::parseName(void)
{
	int ch;
...
void XmlMiniParser::parseDoctype(void)
{
	int ch;
...
void XmlMiniParser::parseEntity(void)
{
	syntaxError();
}
...
void XmlMiniParser::parseNotation(void)
{
	syntaxError();
}
...
void XmlMiniParser::parseElement(void)
{
	syntaxError();
}
...
void XmlMiniParser::parseAttlist(void)
{
	syntaxError();
}
...
void XmlMiniParser::parseConditional(void)
{
	syntaxError();
}
...
void XmlMiniParser::parseComment(void)
{
	int done, ch;
...
void XmlMiniParser::parseCdata(void)
{
	int done, ch;
...
void XmlMiniParser::parsePi(void)
{
	int done, ch;
...
void XmlMiniParser::parseEtag(void)
{
	finishData();
...
void XmlMiniParser::parseStag(void)
{
	char *attval;
	int ch, empty;
...
void XmlMiniParser::parseAttributeName(void)
{
	int ch;
...
char *XmlMiniParser::parseString(void)
{
	int ch, delim;
...
void XmlMiniParser::parseLoop(void)
{
	int ch;
...
int XmlMiniParser::parse(void)
{
	...
	try {
        ...
        if (e != ERR_XMLP_EOF) {
            ...
            mErrorCode = e;
            snprintf(mError, sizeof(mError), "Internal Error %d", mErrorCode);
            Trace(1, "%s\n", mError);
        }
	}
...
void XmlMiniParser::fixHaltPosition(int errcode)
{
