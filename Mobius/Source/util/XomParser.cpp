...
void XomParser::pushStack(XmlNode *n)
{
...
void XomParser::endTag(class XmlMiniParser *p, char *name)
{
	...
	if (!pel || strcmp(pel->getName(), name)) {
		...
		if (!pel)
		  snprintf(buf, sizeof(buf), "Unexpected end tag %s at line %d column %d, "
				  "expecting none.\n",
...
XmlDocument *XomParser::quickParse(const char *xml)
{
...
XmlDocument *XomParser::quickParse(const char *buffer, int len)
{
	XomParser *parser = new XomParser;
	XmlDocument *doc = NULL;
...
XmlDocument *XomParser::parse(const char *buffer, int length)
{
	...
	if (mNode != mDocument) {
		...
		for (s = mStack ; s != NULL ; s = s->getStack()) {
			...
			if (el != NULL) {
				snprintf(buf, sizeof(buf), "Element %s at line %d column %d was "
 ...
 */
XmlDocument *XomParser::parseFile(const char *name)
{
	XmlDocument *retval = NULL;
 ...
 */
int XomParser::getErrorCode()
{
    return mParser->getErrorCode();
}

const char* XomParser::getError()
{
