...
void Vbuf::add(const char *mem, int size)
{
	if (mem != NULL && size) {
...
void Vbuf::addChar(const char ch)
{
...
void Vbuf::addXmlAttribute(const char *value)
{
	const char *ptr;
	int singles, doubles;
	char delim;
...
void Vbuf::addSqlString(const char *value)
{
	addChar('\'');
...
void Vbuf::prepend(const char *text)
{
...
const char *Vbuf::getBuffer(void)
{
	return mBuffer;
}

const char *Vbuf::getString(void)
{
	return mBuffer;
}
