...
class XomStack {
  ...
  public:
	...
	XomStack *pop(void) {
        // new: for some reason had a (this) null guard here
        // and the Mac compiler warned about it
        // shouldn't happen
		// XomStack *neu = (this) ? this->mStack : NULL;
		XomStack *neu = this->mStack;
        delete this;
		return neu;
	}
...
class XomParser : public XmlEventAdapter {
  ...
  public:
	...
	static XmlDocument *quickParse(const char *string);
	static XmlDocument *quickParse(const char *buffer, int len);
	...
	XmlDocument *parse(const char *buffer, int length = 0);
	XmlDocument *parseFile(const char *name);
