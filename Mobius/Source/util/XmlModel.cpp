...
XmlProperty *XmlNode::getPropertyObject(const char *name)
{
	XmlProperty *found, *p;
...
const char *XmlNode::getProperty(const char *name)
{
...
void XmlNode::setProperty(const char *name, const char *value)
{
	XmlProperty *p, *prev;
...
void XmlNode::setChildren(XmlNode *c) 
{
	XmlNode *n;
...
XmlNode *XmlNode::stealChildren(void) 
{
...
XmlElement *XmlNode::findElement(const char *name)
{
...
XmlElement *XmlNode::findElement(const char *elname, 
										   const char *attname,
										   const char *attval)
{
	XmlElement *found, *e;
...
const char *XmlNode::getElementContent(const char *name)
{
	XmlElement *el;
	const char *content;
...
XmlElement *XmlNode::getChildElement(void)
{
	XmlNode *node;
	XmlElement *el;
...
XmlElement *XmlNode::getNextElement(void) 
{
	XmlNode *n;
	XmlElement *el;
 ...
 */
bool XmlElement::isName(const char *name)
{
...
XmlAttribute *XmlElement::getAttributeObject(const char *name)
{
...
int XmlElement::getIntAttribute(const char *name, int dflt)
{
	int value = dflt;
	XmlAttribute *att = getAttributeObject(name);
	if (att != NULL) {
...
int XmlElement::getIntAttribute(const char *name)
{
	return getIntAttribute(name, 0);
}

bool XmlElement::getBoolAttribute(const char *name)
{
	bool value = false;
	XmlAttribute *att = getAttributeObject(name);
	if (att != NULL) {
		const char* v = att->getValue();
        // support "1" for misconfigured objects that
        // had this stored as an int
...
// convenience
void XmlElement::setAttributeInt(const char *name, int value)
{
	char buf[80];
...
const char *XmlElement::getContent(void)
{
...
XmlElement *XmlElement::findNextElement(const char *name)
{
	XmlElement *found;
	XmlNode *sib;
...
XmlAttribute *XmlAttribute::copy(void)
{
	XmlAttribute *copy;
...
XmlProperty *XmlProperty::copy(void)
{
...
void XmlDocument::visit(XmlVisitor *v)
{
	v->visitDocument(this);
}

void XmlDoctype::visit(XmlVisitor *v)
{
	v->visitDoctype(this);
}

void XmlElement::visit(XmlVisitor *v)
{
	v->visitElement(this);
}

void XmlPi::visit(XmlVisitor *v)
{
	v->visitPi(this);
}

void XmlComment::visit(XmlVisitor *v)
{
	v->visitComment(this);
}

void XmlMsect::visit(XmlVisitor *v)
{
	v->visitMsect(this);
}

void XmlPcdata::visit(XmlVisitor *v)
{
	v->visitPcdata(this);
}

void XmlEntref::visit(XmlVisitor *v)
{
	v->visitEntref(this);
}
...
void XmlProperty::visit(XmlVisitor *v)
{
...
char *XmlWriter::exec(XmlNode *node)
{
...
void XmlWriter::visitDocument(XmlDocument *obj)
{
	XmlNode *n;
...
void XmlWriter::visitDoctype(XmlDoctype *obj)
{
	mBuf->add("<!DOCTYPE ");
...
void XmlWriter::visitElement(XmlElement *obj)
{
	XmlAttribute *a;
...
void XmlWriter::visitAttribute(XmlAttribute *obj)
{
...
void XmlWriter::visitPi(XmlPi *obj)
{
	mBuf->add("<?");
...
void XmlWriter::visitComment(XmlComment *obj)
{
	mBuf->add("<!--");
	mBuf->add(obj->getText());
	mBuf->add("-->");
...
void XmlWriter::visitMsect(XmlMsect *obj)
{
	XmlNode *n;
...
void XmlWriter::visitPcdata(XmlPcdata *obj)
{
	mBuf->add(obj->getText());
}

void XmlWriter::visitEntref(XmlEntref *obj)
{
	if (obj->isParameter())
	  mBuf->add("%");
	else
...
void XmlWriter::visitProperty(XmlProperty *obj)
{
    (void)obj;
...
char *XmlNode::serialize(int indent)
{
	XmlWriter *w;
	char *xml;
