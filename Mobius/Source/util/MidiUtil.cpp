 ...
 * 
 * This was broken out of a much larger MIDI interface library.
 * Juce has similar things but I don't want to depend on them yet for
 * core code.
 */
...
void MidiNoteName(int note, char *buf)
{
    ...
    // switched to snprintf to avoid Mac warning, but we don't know
    // the true size at this point and I'm not messing with code this old
    // these are typically larger than necessary
	snprintf(buf, 16, "%s%d", notenames[note], octave - 2);
}
