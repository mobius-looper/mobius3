...
short SampleFloatToInt16(float sample)
{
...
void Audio::dump(StructureDumper& d)
{
    d.line("Audio");
    d.inc();
    d.line("buffers", mBufferCount);
    d.line("version", mVersion);
    d.line("startFrame", (int)mStartFrame);
    d.line("frames", (int)mFrames);
           
    d.dec();
}

AudioPool* Audio::getPool()
{
    return mPool;
}
...
void Audio::setSampleRate(int i) 
{
	mSampleRate = i;
}
...
void Audio::locate(long frame, int* buffer, int* offset)
{
	...
	*buffer = (int)(sample / mBufferSize);
	*offset = (int)(sample % mBufferSize);
}
 ...
 */
void Audio::splice(long frame, long length)
{
 ...
 */
void Audio::setFrames(long frames) 
{
 ...
 */
void Audio::setFramesReverse(long frames)
{
...
void Audio::setStartFrame(long frame)
{
	if (frame >= 0) {
		...
		int needBuffers = (int)(needSamples / mBufferSize);
 ...
 */
int Audio::read(const char *name) 
{
	int error = 0;
 ...
 */
void Audio::get(AudioBuffer* buf, long frame)
{
	mPlay->setFrame(frame);
    mPlay->get(buf, 1.0);
}

void Audio::get(float* dest, long frames, long frame)
{
	AudioBuffer b;
	b.buffer = dest;
	b.frames = frames;
...
void Audio::insert(Audio* audio, long insertFrame)
{
    if (audio != NULL && audio->getSamples() > 0) {
        if (insertFrame >= mFrames) {
			...
			int shiftSamples = (int)((mFrames - insertFrame) * mChannels);
