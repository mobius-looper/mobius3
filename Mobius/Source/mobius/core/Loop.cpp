...
/**
 * Export the full loop state for UI access.
 * Note that this is not called in the audio thread, it is usually the
 * maintenance thread so some things could be changing out from under us
 * as we go.
 *
 * NOTE: Old code had two models, the "full" and the "summary".  We just
 * have the full model in MobiusLoopState now, so some of the things
 * set might not be relevant if this isn't the active loop.
 *
...
void Loop::refreshState(OldMobiusLoopState* s, bool active)
{
	...
	s->paused = mPause;
    // these will be set by EventManager if there are Switch events
	s->nextLoop = -1;
    s->returnLoop = -1;
	s->overdub = mOverdub;
    ...
    if (active) {
        ...
        // forget why there was a difference on these for non-active loops
		// relevant only for the active track
		s->mute = false;

		if (mPreset->getReverseTransfer() == XFER_RESTORE)
		  s->reverse = mRestoreState.reverse;
		else
		  s->reverse = false;
		
		if (mPreset->getSpeedTransfer() == XFER_RESTORE) {
		...
		if (mPreset->getPitchTransfer() == XFER_RESTORE) {
    ...
    if (active ) {
        ...
        // summary didn't have these, clear them?
        s->beatLoop = false;
        s->beatCycle = false;
        s->beatSubCycle = false;
    }
    ...
    // layers only for the active loop
    if (active) {
    ...
    // old flag, not really relevant, weed out usages
    s->summary = !active;

    // track will fill this in after we're done
    s->eventCount = 0;
}
