...
void MultiplyFunction::doEvent(Loop* l, Event* e)
{
    ...
    if (e->type == MultiplyEvent) {
        ...
        if (l->isUnroundedEnding(e->getInvokingFunction())) {
            ...
            if (cycleFrames > 0) {
                int saveCycles = (int)(multiplyLength / cycleFrames);
                if (saveCycles > 0) {
...
void MultiplyFunction::pruneCycles(Loop* l, int cycles, 
                                   bool forceUnrounded, bool remultiply)
{
    OutputStream* output = l->getOutputStream();
	...
	sync->loopResize(l, forceUnrounded);
	...
	if (modeStartFrame > 0) {
		...
		if (output->getLastLayer() == l->getPlayLayer())
		  output->adjustLastFrame((int)(-modeStartFrame));
	}
