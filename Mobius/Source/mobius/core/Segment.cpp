...
void Segment::dump(StructureDumper& d)
{
    d.line("Segment");
    d.inc();
    d.start("");
    d.add("offset", (int)mOffset);
    d.add("startFrame", (int)mStartFrame);
    d.add("frames", (int)mFrames);
    d.add("layer", mLayer->getNumber());
    d.newline();
    d.dec();
}

void Segment::dumpList(StructureDumper& d)
{
    dump(d);
    Segment* next = mNext;
    while (next != nullptr) {
