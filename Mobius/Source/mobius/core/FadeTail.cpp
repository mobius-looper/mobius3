...
FadeTail::FadeTail()
{
	...
	int tailSamples = (int)(mMaxFrames * AUDIO_MAX_CHANNELS);
...
void FadeTail::add(float* tail, long frames)
{
	if (frames > 0) {
...
#if 0
		if (trace) {
			snprintf(file, sizeof(file), "addTail%d.wav", AddTailCount);
		...
		if (newFrames > mFrames)
		  mFrames = (int)newFrames;
#if 0
		if (trace) {
			snprintf(file, sizeof(file), "trackTail%d.wav", AddTailCount);
