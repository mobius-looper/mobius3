...
SyncMaster::RequestResult SyncMaster::requestSwitchRecord(int number, int blockRemaining)
{
    ...
    if (result.synchronized) {
        ...
        if (lt != nullptr) {
            ...
            lt->setSyncRecordElapsedFrames(blockRemaining);
        }
    }
...
Pulse* SyncMaster::getBlockPulse(LogicalTrack* track)
{
    ...
    if (track->isSyncRecording()) {
        ...
        if (!track->isSyncRecordStarted()) {
            ...
            // note that the unit length on the track is the "sync unit length"
            // which is a beat for midi, transport, and host but
            // for track sync it is the same as the TrackSyncUnit which can be any of the three
            // these need to be converted into "record units" for pulses
            
            // !! move these calculations to Unitarian
            int syncUnitLength = track->getUnitLength();
            int recordUnitLength = syncUnitLength;

            SyncSource src = track->getSyncSource();
            if (src != SyncSourceTrack) {
                SyncUnit unit = track->getSyncUnit();
                if (unit == SyncUnitBar) {
                    recordUnitLength *= barTender->getBeatsPerBar(track);
                }
                else if (unit == SyncUnitLoop) {
                    recordUnitLength *= barTender->getBeatsPerBar(track);
                    recordUnitLength *= barTender->getBarsPerLoop(track);
                }
            }
            
            int elapsed = track->getSyncRecordElapsedFrames();
            int lastFrameInBlock = elapsed + blockSize - 1;
            
            if (lastFrameInBlock < recordUnitLength) {
                // unit does not transpire in this block
                track->setSyncRecordElapsedFrames(elapsed + blockSize);
            }
            else {
                ...
                track->setSyncRecordElapsedFrames(overage);
...
void SyncMaster::handleBlockPulse(LogicalTrack* track, Pulse* pulse)
{
    if (!track->isSyncRecordStarted()) {
        ...
        track->setSyncRecordElapsedFrames(blockSize - pulse->blockFrame);
    }
    else {
