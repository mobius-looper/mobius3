/**
 * This is a subcomponent of LooperScheduler that isolates the code surrounding
 * loop switching and helps keep LooperScheduler from being too bloated.
 */
...
LooperSwitcher::~LooperSwitcher()
{
}

void LooperSwitcher::initialize()
{
}
 ...
 */
bool LooperSwitcher::isSwitching()
{
 ...
 */
void LooperSwitcher::scheduleSwitch(UIAction* src)
{
 ...
 */
int LooperSwitcher::getSwitchTarget(UIAction* a)
{
    LooperTrack* track = scheduler.track;
    ...
    if (sid == FuncPrevLoop) {
        ...
        if (a->value < 1 || a->value > track->getLoopCount())
          Trace(1, "LooperSwitcher: Loop switch number out of range %d", target);
        else
          target = a->value - 1;
    }
    return target;
 ...
 */
int LooperSwitcher::getQuantizedFrame(SwitchQuantize squant)
{
 ...
 * options.
 */
QuantizeMode LooperSwitcher::convert(SwitchQuantize squant)
{
 ...
 */
void LooperSwitcher::handleSwitchModeAction(UIAction* src)
{
    ...
    if (ending == nullptr) {
        ...
        Trace(1, "LooperSwitcher: Switch action handler called without a Switch event");
    }
        ...
        if (sid == FuncNextLoop || sid == FuncPrevLoop || sid == FuncSelectLoop) {
            Trace(1, "LooperSwitcher: Ignoring switch function when waiting for a Return");
        }
        else {
            // non-switch actions are simply stacked on the return event and executed later
            Trace(2, "LooperSwitcher: Stacking %s after return switch", src->symbol->getName());
        ...
        if (sid == FuncNextLoop || sid == FuncPrevLoop || sid == FuncSelectLoop) {
            ...
            if (sid == FuncNextLoop) {
                ...
                if (target < 0 || target >= track->getLoopCount())
                  Trace(1, "LooperSwitcher: Loop switch number out of range %d", src->value);
                else
            ...
            Trace(2, "LooperSwitcher: Stacking %s after switch", src->symbol->getName());
            ending->stack(scheduler.copyAction(src));
        }
    }
}
 ...
 */
void LooperSwitcher::doSwitchNow(UIAction* a)
{
 ...
 *
 */
void LooperSwitcher::leaderEvent(TrackProperties& props)
{
    (void)props;
    ...
    if (e == nullptr) {
        ...
        Trace(1, "LooperSwitcher: Leader notification did not find Switch event");
    }
    else if (!e->pending) {
        // Similar to event not found, we allowed the Switch event to be activated
        // without a leader notification
        Trace(1, "LooperSwitcher: Leader notification found an active Switch event");
    }
    else {
 ...
 * Scheduler.
 */
void LooperSwitcher::doSwitchEvent(TrackEvent* e, int target)
{
    LooperTrack* track = scheduler.track;
    ...
    if (e == nullptr || !e->isReturn) {
        ...
        if (duration != SWITCH_PERMANENT && isRecording) {
            ...
            Trace(1, "LooperSwitcher: Ignoring SwitchDuration after starting record of empty loop");
        }
            ...
            Trace(2, "LooperSwitcher: Ignoring SwitchDuration after switching to empty loop");
        }
        else {
            switch (duration) {
                case SWITCH_ONCE: {
                ...
                case SWITCH_SUSTAIN: {
                    ...
                    Trace(1, "LooperSwitcher: SwitchDuration=Sustain not implemented");
                }
                    break;
                case SWITCH_SUSTAIN_RETURN: {
                    // I think this is supposed to do a Return when the trigger goes up
                    Trace(1, "LooperSwitcher: SwitchDuration=SustainReturn not implemented");
                }
                    break;
    ...
    if (e != nullptr && e->stacked != nullptr && isRecording) {
        Trace(2, "LooperSwitcher: Ignoring stacked actions after empty loop record");
    }
    else {
 ...
 *
 */
bool LooperSwitcher::setupEmptyLoop(int previousLoop)
{
    LooperTrack* track = scheduler.track;
