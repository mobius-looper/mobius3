 ...
 */
float AudioFade::fade(float sample)
{
	if (active) {
		float* ramp = getRamp();
 ...
 */
void AudioFade::inc(long frame, bool reverse)
{
	if (active) {
		processed++;
		if (processed >= Range) {
...
{
	char name[1024];
	snprintf(name, sizeof(name), "fade-%d-%s-%s.wav",
			FadeCount,
			((up) ? "up" : "down"),
			type);
 ...
 */
void AudioFade::fade(AudioBuffer* buf, long curFrame)
{
	...
	if (fadeEndFrame < curFrame) {
		...
		if (trace) {
			...
			snprintf(filename, sizeof(filename), "fade-before-%d.wav", FadeCount);
			save->write(filename);
		}
#endif
 ...
 */
void AudioFade::fade(float* buffer, int channels, 
							...
							long fadeOffset, bool up)
{
	...
	float* ramp = getRamp();
	long rampIndex = ((up) ? fadeOffset : (Range - fadeOffset - 1));
	int incIndex = ((up) ? 1 : -1);
 ...
 */
void AudioFade::fadePartial(float* buffer, int channels, 
								   long startFrame, long frames, 
								   long fadeOffset, bool up,
								   float baseLevel)
{
...
void AudioCursor::setAutoExtend(bool b)
{
	mAutoExtend = b;
}
 ...
 */
void AudioCursor::setFrame(long frame)
{
    if (frame != mFrame) {
		mFrame = (int)frame;
		decache();
 ...
 */
void AudioCursor::get(AudioBuffer* buf, float level)
{
...
void AudioCursor::get(AudioBuffer* buf)
{
	get(buf, 1.0);
}

void AudioCursor::get(AudioBuffer* buf, Audio* a, long frame, 
							 float level)
{
	setAudio(a);
	setFrame(frame);
 ...
 * some content.
 */
void AudioCursor::get(AudioBuffer* buf, float* dest, float level)
{
...
void AudioCursor::put(AudioBuffer* buf, AudioOp op)
{
	int channels = buf->channels;
...
void AudioCursor::put(AudioBuffer* buf, AudioOp op, long frame)
{
	setFrame(frame);
	put(buf, op);
}

void AudioCursor::put(AudioBuffer* buf, AudioOp op, Audio* a, 
							 long frame)
{
	setAudio(a);
	setFrame(frame);
 ...
 */
void AudioCursor::setFadeIn(long frame)
{
	mFade.enable(frame, true);
}

bool AudioCursor::isFading()
{
	return mFade.active;
}
 ...
 * Deprecated: we no longer use "scheduled" fades.
 */
void AudioCursor::setFadeOut(long frame)
{
...
void AudioCursor::transferFade(AudioCursor* dest)
{
	dest->mFade = this->mFade;
	mFade.init();
}

void AudioCursor::resetFade()
{
    mFade.init();
}
 ...
 */
void AudioCursor::fade(int offset, int frames, bool up)
{
 ...
 */
void AudioCursor::fadeIn()
{
	setFrame(0);
	fade(true);
}

void AudioCursor::fadeIn(Audio* a)
{
	setAudio(a);
    fadeIn();
}
...
void AudioCursor::fadeOut(Audio* a)
{
    setAudio(a);
    fadeOut();
}
