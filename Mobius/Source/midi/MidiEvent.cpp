...
/**
 * A very old model for MIDI events.
 * This will eventually be replaced with juce::MidiMessage when there
 * aren't more important things to do.
 *
 * These used to have an XML serialization using the XomParser which
 * has been commented out.
 *
 * Things related to lists of events were for an ancient sequencer that
 * no longer exists and won't be carried forward.
 */
...
MidiEvent::~MidiEvent(void)
{
    ...
    for (el = mNext, nextel = nullptr ; el != nullptr ; el = nextel) {
        nextel = el->mNext;
        el->mNext = nullptr;	// prevent recursion
        delete el;
    }
}
...
MidiEvent *MidiEvent::getNextEvent(void)
{
	...
	next = nullptr;
	if (this != nullptr) {
		for (next = this->getNext() ; next != nullptr ; next = next->getNext()) {
...
MidiEvent *MidiEvent::insert(MidiEvent *neu)
{
	...
	if (neu->mStatus != MS_CMD_LOOP) {
		while (event != nullptr && event->mClock == neu->mClock && 
