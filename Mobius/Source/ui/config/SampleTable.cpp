...
/**
 * Populate internal state with a list of Samples from a SampleConfig.
 */

void SampleTable::setSamples(SampleConfig* config)
{
    files.clear();
    ...
    while (sample != nullptr) {
        ...
        // ugh, we do however support $INSTALL
        if (path.startsWith(InstallationPathPrefix)) {
            path = path.fromFirstOccurrenceOf(InstallationPathPrefix, false, false);
...
void SampleTable::updateContent()
{
    table.updateContent();
}

/**
 * Reconstructes a SampleConfig from the SampleFiles
 * Ownership of the SampleConfig passes to the caller.
 */
SampleConfig* SampleTable::capture()
{
    SampleConfig* config = new SampleConfig();
    for (int i = 0 ; i < files.size() ; i++) {
        SampleFile* sf = files[i];
        if (sf->path.length() > 0) {
            Sample* sample = new Sample(sf->path.toUTF8());
            config->add(sample);
        }
    }
    return config;
...
int SampleTable::getPreferredWidth()
{
    // adapt to column configuration
    return 500;
}

int SampleTable::getPreferredHeight()
{
    int height = 400;
 ...
 * ButtonBar::Listener
 */
void SampleTable::buttonClicked(juce::String name)
{
...
juce::String SampleTable::getCellText(int rowNumber, int columnId)
{
    (void)columnId;
    // only one column
 ...
 */
void SampleTable::paintRowBackground(juce::Graphics& g, int rowNumber,
 ...
 * 14 is 63% of 22
 */
void SampleTable::paintCell(juce::Graphics& g, int rowNumber, int columnId,
                             int width, int height, bool rowIsSelected)
{
    (void)columnId;
    SampleFile* file = files[rowNumber];
 ...
 */
void SampleTable::cellClicked(int rowNumber, int columnId, const juce::MouseEvent& event)
{
    (void)rowNumber;
    (void)columnId;
    (void)event;
 ...
 * you are, but how you got there is magic.  Here we'll call SampleTable's
